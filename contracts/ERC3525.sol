...
contract ERC3525 is Context, IERC3525Metadata, IERC721Enumerable {
    ...
    function _isApprovedOrOwner(address operator_, uint256 tokenId_) public view virtual returns (bool) {
        address owner = ERC3525.ownerOf(tokenId_);
        return (
            operator_ == owner ||
    ...
    function _spendAllowance(address operator_, uint256 tokenId_, uint256 value_) public virtual {
    ...
    function _exists(uint256 tokenId_) public view virtual returns (bool) {
        return _allTokens.length != 0 && _allTokens[_allTokensIndex[tokenId_]].id == tokenId_;
    }

    function _requireMinted(uint256 tokenId_) public view virtual {
    ...
    function _setMetadataDescriptor(address metadataDescriptor_) public virtual {
        metadataDescriptor = IERC3525MetadataDescriptor(metadataDescriptor_);
        emit SetMetadataDescriptor(metadataDescriptor_);
    }

    function _createOriginalTokenId() public virtual returns (uint256) {
        return _tokenIdGenerator++;
    }

    function _createDerivedTokenId(uint256 fromTokenId_) public virtual returns (uint256) {
        fromTokenId_;
